...
void Moteur3D::afficherTriangle(const Modele * modele, unsigned int s, Point2D * p1, Point2D * p2, Point2D * p3, float ang, float distMin)&
{
	...
	a0d=1.0*m0x/(cam->getDistFoc()*cam->getTaillex());
	a1d=1.0*m1x/(cam->getDistFoc()*cam->getTaillex());
	a2d=1.0*m2x/(cam->getDistFoc()*cam->getTaillex());
	in1=a0d*m2z-a2d*m0z;
	jn1=a2d*m0y-a0d*m2y;
	n1=m2y*m0z-m0y*m2z-in1/2.0*cam->getTaillex()+jn1/2.0*cam->getTailley();
	in1=in1*cam->getTaillex()/imageTaillex;
	jn1=-jn1*cam->getTailley()/imageTailley;
	in2=a1d*m0z-a0d*m1z;
	jn2=a0d*m1y-a1d*m0y;
	n2=m0y*m1z-m1y*m0z-in2/2.0*cam->getTaillex()+jn2/2.0*cam->getTailley();
	in2=in2*cam->getTaillex()/imageTaillex;
	jn2=-jn2*cam->getTailley()/imageTailley;
	id=a2d*m1z-a1d*m2z;
	jd=a1d*m2y-a2d*m1y;
	d=m1y*m2z-m2y*m1z-id/2.0*cam->getTaillex()+jd/2.0*cam->getTailley();
	id=id*cam->getTaillex()/imageTaillex;
	jd=-jd*cam->getTailley()/imageTailley;
